...
class CultivationPersonListAdapter constructor(private val context: Context, private val list: MutableList<Person>) : BaseAdapter() {
    ...
    override fun getView(arg0: Int, convertViews: View?, arg2: ViewGroup): View {
        ...
        component.name.text = "${CultivationHelper.showing(person.name)}$lifeTurn(${CultivationHelper.showing(person.gender.props)})${if(person.teji.isEmpty()) "" else person.teji.size}"
